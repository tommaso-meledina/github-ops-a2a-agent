...
def log_response(resp):
    """
    Log only the user-relevant text from an A2A response object,
    by pulling text out of parts rather than dumping the entire model.
    """
    if hasattr(resp, 'root'):
        resp = resp.root
    # If this is a Message
    if hasattr(resp, "message"):
        print(f"A: {get_message_text(resp.message)}")
        return

    # If this is a Task containing artifacts
    result = getattr(resp, "result", None)
    if result and hasattr(result, "artifacts") and result.artifacts is not None:
        for artifact in result.artifacts:
            print(f"A: {get_message_text(artifact)}")
        return

    # Fallback if unexpected
    print(f"No text content found in response, here is a dump:\n{resp.model_dump(mode='json', exclude_none=True)}")
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        second_query = 'fetch the contents of issue 1'
        ...
        third_query = 'repo name is "github-ops-mcp-server", owner is "tommaso-meledina"'

        second_send_message_payload_multiturn: dict[str, Any] = {
            'message': {
                'role': 'user',
                'parts': [{'kind': 'text', 'text': third_query}],
                'message_id': uuid4().hex,
                'task_id': task_id,
